...
const analyzeIssues = (text) => {
  ...
  return Object.entries(ISSUE_INDICATORS)
    .filter(([_, keywords]) =>
      keywords.some((kw) => lowercaseText.includes(kw))
    )
    .map(([issue]) => issue);
};
...
const analyzeSpeed = (text) => {
  const lowercaseText = text.toLowerCase();
  const hasFast = SPEED_INDICATORS.fast.some((word) =>
    lowercaseText.includes(word)
  );
  const hasSlow = SPEED_INDICATORS.slow.some((word) =>
    lowercaseText.includes(word)
  );
      ...
      Review.aggregate([
        ...
        },
        {
          $project: {
            ...
            totalReviews: 1,
            issueRate: {
              $multiply: [{ $divide: ["$issueCount", "$totalReviews"] }, 100],
            },
          },
        },
        { $sort: { avgRating: -1 } },
